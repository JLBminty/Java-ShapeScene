 ...
 * Jordan Burns
 * February 1 2024
 * ADDED PRODUCTIONS FOR TEXT, ISOCELESTRIANGLE, PARALELLOGRAM, AND REGULAR POLYGON
 */
...
class Parser {
    ...
    // ADDED PRODUCTIONS TO THE PARSER:
    // image -> right_triangle | rectangle | text | isocelese | parallelogram | regular_polygon
    // text -> TEXT COLOR number_list AT number_list EOF ';'
    // isocelese -> ISOCELES_TRIANGLE COLOR number_list AT number_list HEIGHT NUMBER WIDTH NUMBER ';'
    // parallelogram -> PARALELLOGRAM COLOR number_list AT number_list number_list OFFSET NUMBER ';'
    // regular -> REGULAR_POLYGON COLOR number_list AT number_list SIDES NUMBER RADIUS NUMBER ';'
    ...
    private void parseImages(Scene scene, Token imageToken) throws LexicalError, SyntaxError, IOException {
        ...
        case RIGHT_TRIANGLE:
            ...
            RightTriangle rTriangle = new RightTriangle(color, point, height, width);
            scene.addImage(rTriangle);
            break;
        ...
        case ISOSCELES:
            ...
            verifyNextToken(Token.NUMBER);
            width = lexer.getNumber();
            IsoscelesTriangle iTriangle = new IsoscelesTriangle(color, 3, point, height, width);
            scene.addImage(iTriangle);
            break;
            
        case PARALLELOGRAM:
        	int[] lowerRightLocation = getNumberList(2);
        	Point rightPoint = new Point(lowerRightLocation[0], lowerRightLocation[1]);
        	verifyNextToken(Token.OFFSET);
        	verifyNextToken(Token.NUMBER);
        	offset = lexer.getNumber();
        	Parallelogram parallelogram = new Parallelogram(color, 4, point, rightPoint, offset);
        ...
        case REGULARPOLYGON:
        	...
        	RegularPolygon regular = new RegularPolygon(color, sides, point, radius);
        	scene.addImage(regular);
        	break;
        	
        case TEXT:
        	verifyNextToken(Token.EOF);
        	String phrase = lexer.getString();
        	Text text = new Text (color, phrase, point);
        	scene.addImage(text);
        	break;
    ...
    // Returns an array of the numbers in the number list (integers only)
